...
# SỰ KHÁC NHAU KHI TẠO THREAD

Bạn hỏi rất đúng. Câu “tách logic công việc ra khỏi Thread” nghe hơi lý thuyết, mình nói lại bằng code cho dễ hiểu.
...
Ở cách này, class `DownloadThread` đang làm **2 vai trò cùng lúc**:

```text
1. Nó là Thread
2. Nó chứa logic download
```

Tức là **công việc** và **thread chạy công việc** bị gộp chung vào một class.
...
Nhìn tên class cũng thấy: `DownloadThread` vừa là “download task”, vừa là “thread”.
...
Ở đây `DownloadTask` chỉ làm **1 việc duy nhất**:

```text
Nó mô tả công việc cần làm
```

Nó **không phải Thread**.

Muốn chạy công việc đó bằng thread nào thì đưa vào `Thread`:
...
Lúc này vai trò được tách rõ:

```text
DownloadTask -> công việc cần chạy
Thread       -> luồng thực thi công việc đó
```
...
```text
DownloadThread = vừa là người công nhân, vừa là công việc tải file
```

Nó bị dính chặt vào Thread.

### Cách 2: Runnable

```text
DownloadTask = phiếu công việc
Thread       = người công nhân cầm phiếu đó đi làm
```

Một công việc có thể đưa cho nhiều kiểu xử lý khác nhau.

---

## Vì sao Runnable linh hoạt hơn?

### 1. Class vẫn có thể `extends` class khác

Java không cho đa kế thừa class.

Nếu dùng `extends Thread`:
...
thì class này **không thể extends class khác nữa**.

Ví dụ không làm được:

```java
class DownloadTask extends BaseTask, Thread { // sai
}
```

Nhưng nếu dùng `Runnable`:

```java
class DownloadTask extends BaseTask implements Runnable {
    @Override
    public void run() {
        System.out.println("Downloading file...");
    }
...
### 2. Runnable dùng được với `ExecutorService`

Trong thực tế ít khi tạo thread thủ công quá nhiều bằng:

```java
new Thread(task).start();
```

Người ta thường dùng thread pool:
...
`ExecutorService` nhận `Runnable` rất tự nhiên.

Nếu bạn viết task bằng `Runnable`, bạn có thể chạy nó bằng:
...
Còn nếu class của bạn `extends Thread`, nó bị gắn chặt với `Thread`.
...
```text
extends Thread
-> Class chính nó là một thread
-> Logic công việc và thread bị gộp chung
-> Không thể extends class khác
-> Ít linh hoạt hơn

implements Runnable
-> Class chỉ mô tả công việc cần chạy
-> Thread chỉ là nơi chạy công việc
-> Vẫn có thể extends class khác
-> Dùng tốt với ExecutorService
-> Recommended hơn
```

## Code so sánh rõ nhất
...
```text
DownloadTask là công việc
Thread là luồng chạy công việc
```

---

## Câu phỏng vấn dễ nói

> Với `extends Thread`, class vừa đại diện cho thread vừa chứa logic công việc, nên bị gắn chặt với Thread và kém linh hoạt. Còn với `Runnable`, class chỉ mô tả công việc cần chạy trong method `run()`, sau đó công việc này có thể được chạy bởi `Thread` hoặc `ExecutorService`. Vì vậy Runnable giúp tách task logic khỏi thread management và thường được khuyên dùng hơn.
